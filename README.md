...
Localizing molecules in large images can generate a huge amounts of data to process. Analysing these point clouds and trajectories to detect localization errors can therefore be very time consuming. This is what motivated the combination of two platforms: **DIVA** and **Genuage**, in order to visualize raw image in virtual reality, overlayed with the trajectory data, thus allowing users to explore and check the validity of multidimensional point cloud data.
...
- [Navigating data in Virtual Reality](#navigating-data-in-virtual-reality)
...
DIVA is designed to run on the Windows 10 and 11 and requires OpenCL 2.0 or higher. We recommend using DIVA on a machine with the following minimum requirement : 
...
> *Note*: if you see this type of behavior after loading your image in DIVA, press R on your keyboard to reload the image and correct the rendering artefacts.
...
## Data requirements
DIVA requires a trajectory file in .csv format containing five colums: ```trajectory id```, ```frame index```, ```x```, ```y```, ```z``` where each variable is separated by a comma. Point coordinates have to be in the image coordinates system (without normalization between 0 and 1). 
...
After loading, each point in the file will be added to the volume and rendered as Gaussian spheres as well as the trajectories which will be rendered as lines connecting the points together. 
...
> /!\ The button will not respond if SteamVR is not installed. To avoid any possible crash, launch SteamVR before launching DIVA.
...
- To select a trajectory, go to the position of a point of the trajectory and click on the <img src="https://user-images.githubusercontent.com/49953723/211562957-e70b53c8-d312-4a9d-b4e5-0bf6cc12a2bd.png" width="20px"/> icon. 
- To remove a trajectory from the list of selected trajectories, go to the position of a point of the trajectory and click on the <img src="https://user-images.githubusercontent.com/49953723/211562979-fc2154d8-221b-4f11-bb45-ce31c771bf39.png" width="20px"/> icon. 

Ids of selected trajectories are displayed under the panel ```Selected Trajectories``` on the desktop interface.

 > *Note*: a point in the cloud is highlited in green when the controller passes over it. 

## Export selected trajectories
Exportation of the selected trajectories can be done on the desktop interface by clicking the <img src="https://user-images.githubusercontent.com/49953723/211569177-e09271e8-0930-4489-99e1-ea765edc5633.png" width="170px"/> button. Data is exported as .csv file with the same format as the input file and contains the list of points of the selected trajectories.
